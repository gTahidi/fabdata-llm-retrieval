...
class DataStore(ABC):
    async def upsert(
        ...
        chunks: Optional[Dict[str, List[DocumentChunk]]] = None
    ) -> List[str]:
        """
        ...
        if chunks is None:
            ...
            or any(document.id not in chunks for document in documents)
        ):
            raise ValueError("Chunks if provided must match documents")
